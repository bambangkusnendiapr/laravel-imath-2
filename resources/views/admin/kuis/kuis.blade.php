...
<section class="section">
    <div class="section-header">
        ...
        <div class="section-header-breadcrumb">
          <div class="breadcrumb-item active"><a href="#">Manajemen Kuis</a></div>
        ...
        <div class="col-md-12 col-lg-12 col-sm-12">
          <div class="card">
            <div class="card-header">
             <a href="{{ route('kuis.create')}}" class="btn btn-warning btn-sm"> <span data-feather="plus" style="width: 15px;"></span> Tambah Kuis</a>
            </div>
            <div class="card-body">
                      ...
                      <tbody>
                        @foreach ($kuiss as $kuis)
                            <tr>
                                ...
                                <td>{{$kuis->status}}</td>
                                <td>{{$kuis->tgl_aktif}}</td>
                                <td>
                                    <a href="{{ route('kuis.edit',$kuis->id)}}" class="btn btn-success btn-sm"><span data-feather="edit" style="width: 15px;"></span> Sunting</a>
                                    <form action="/kuis/{{$kuis->id}}" method="POST" class="d-inline">
                                        @method('delete')
                                        @csrf
                                        <button class="btn btn-danger border-0 btn-sm" onclick="return confirm('Yakin Mau Hapus Kuis Ini?')"><span data-feather="trash" style="width: 15px;"></span> Hapus</button>
                                      </form>
                                </td>
                            </tr>
                        @endforeach
